	...
	def inline_keyboards(self):
		...
		def Confirm(Call: types.CallbackQuery):
			...
			Text = (
				...
				_("Если вы находитесь в хорошем настроении, то напишите ещё что-то. Вам это вернётся в 10 раз больше 😊!")
			)
			TeleMaster(bot).safely_delete_messages(Call.from_user.id, Call.message.id)
			bot.send_message(
		...
		def Message(Call: types.CallbackQuery):
			...
			UserOptions.add_removable_messages(
				bot.send_message(
					...
					reply_markup = ExchangerInlineTemplates.thank_you(_("Спасибо, попозже придумаю!"))
				).id
			)
...
class Procedures:
	...
	def text(self, message: types.Message) -> bool:
		...
		if LENGTH <= 200:
			...
			Text = (
				_("Проверьте, пожалуйста, все ли вы правильно написали. <b>Ваш текст</b>:"),
				message.text
			)
			UserOptions.add_removable_messages(
				bot.send_message(
